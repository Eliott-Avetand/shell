...
hello()
{
  echo -e "\n     ${BLUE}Hello ! I'm Emile your security robot${NC}
\t         __
\t _(\    |@@|
\t(__/\__ \--/ __
\t   \___|----|  |   __
...
load_test()
{
  ...
  then
    ...
    then
      verify_category "$id"
      echo -e "$id. $NAME > ${RED}KO${NC} - Check output in /tmp/test.$$/$id/"
...
if [ ! -f tests ]
then
  echo "No tests file. Please read README_TESTER" >&2
  exit 1
fi
